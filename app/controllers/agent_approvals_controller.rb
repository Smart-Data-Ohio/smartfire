class AgentApprovalsController < ApplicationController
  ...
  def update
    ...
    if decision == "approved" && !@approval.approvable_by?(Current.user)
      service = @approval.fizzy_action? ? "Fizzy" : "GitHub"
      message = "Only an administrator can approve #{service} write actions"
      respond_to do |format|
    ...
    if decision == "approved" && @approval.fizzy_action? && !@approval.fizzy_identity_current?
      message = "The agent owner's Fizzy account changed since this was requested; deny it and ask the agent to request again"
      respond_to do |format|
        format.html { redirect_back_or_to activity_items_path, alert: "#{message}.", status: :see_other }
        format.json { render json: { error: message }, status: :unprocessable_entity }
      end
    ...
    begin
      ...
      AuditLog.record!(action: "agent.approval.decide", target: @approval,
        changes: { decision: AuditLog.pair("pending", decision), note: @approval.decision_note }.compact)
