...
DEST_DIR="/opt/campfire-backups"
ENV_DIR="/etc/campfire-backups"
...
install -o root -g root -m 0644 "$SRC_DIR/campfire-backup.service" /etc/systemd/system/campfire-backup.service
install -o root -g root -m 0644 "$SRC_DIR/campfire-backup.timer" /etc/systemd/system/campfire-backup.timer
...
# Warn, don't fail, when the configured encryption tool is missing: the lead
# may install age after this script (apt-get install age) and before the
# first timer run.
...
if [ "$encryption" = "gpg" ]; then
  ...
  command -v age >/dev/null || log "WARNING: age is configured but not installed (apt-get install age)"
fi
