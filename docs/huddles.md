...
Starting a call is not the only event worth hearing about: when Chris joins a huddle Riel is already in, Riel sees a small in-call toast, "Chris joined". Rapid joins batch into one toast ("Chris and Dean joined") with a single subtle blip, played through the same `sound` controller as every chat sound, so DND, quiet hours, meeting quiet, and out-of-office silence it the same way. Leaving toasts quietly in the call too ("Chris left"), with no sound. These toasts fire in every room kind — DMs, channels, and voice rooms — but only for members currently in the call. A leave waits about five seconds before toasting, so a server mute — which revokes the grant and rejoins it automatically — never flashes "left" followed by "joined": a join from the same person inside the window cancels the pending leave and stays silent itself. The pending leave survives Turbo navigations, so a mute cycle during a room switch stays silent too.
