...
| Two-step | `sign_in.two_factor.failure` (wrong code at the challenge), `two_factor.enable`, `two_factor.disable`, `two_factor.reset`, `two_factor.backup_codes.regenerate`, `two_factor.reauthenticate` (Google step-up for sensitive self-service), `two_factor.devices.revoke_all`. Codes and secrets never reach the log. |
